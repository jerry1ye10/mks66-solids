#Thanks to ivan zhang(pd 04) for help with this assignment
from display import *
from matrix import *
from gmath import *
import random
...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    z_f = middle[2]
    if bottom[1]!=top[1]:
        delta_x = (top[0]-x)/(top[1]-bottom[1])
    else:
        delta_x = 0
    if middle[1]!=bottom[1]:
        delta_x_f = (middle[0]-x)/(middle[1]-bottom[1])
    else:
        delta_x_f = 0
    if top[1]!=bottom[1]:
        delta_z = (top[2]-z)/(top[1]-bottom[1])
    else:
        delta_z = 0
    if middle[1]!=bottom[1]:
        delta_z_f = (middle[2]-z)/(middle[1]-bottom[1])
    else:
        delta_z_f = 0

    while y<middle[1]:
        draw_line(int(x), int(y), int(z), int(x_f), int(y), int(z_f), screen, zbuffer, color)
        x += delta_x
        x_f += delta_x_f
        z += delta_z
        z_f += delta_z_f
    ...
    if middle[1]!=top[1]:
        delta_x_f = (top[0]-x_f)/(top[1]-middle[1])
    else:
        delta_x_f = 0
    if middle[1]!=top[1]:
        delta_z_f = (top[2]-z_f)/(top[1]-middle[1])
    else:
        delta_z_f = 0
    while top[1]>y:
        draw_line(int(x), int(y), int(z), int(x_f), int(y), int(z_f), screen, zbuffer, color)
